...
from app.controllers.user_controllers import logout_user, login_user, update_user, toggle_user_status, get_user_logs
...
def create_user():
    ...
    #Registra un cambio en nuestra db, un commit tipo github

    db.session.commit()

    return jsonify({"msg":"Usuario creado con éxito", "user":new_user.to_dict()}),200
...
def login():
    ...
    if token:
        return jsonify({"acces token":token}),200
...
def get_logs(user_id):
    ...
    for log in logs:
        log.date = log.date.isoformate()
        data.append(log.to_dict())
    return jsonify({"msg":"Logs obtenidos con éxito", "logs":data}),200
