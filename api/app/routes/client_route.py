...
from app.controllers.client_controler import create_client, search_client_by_name, search_client_by_phone, update_client, delete_client
...
def update(client_id):
    ...
    if not client:
        return jsonify({"error":"Cliente no encontrado"}), 400
    return jsonify("CLIENTE ACTUALIXADO CON ÉXITO"), 200
...
def delete(client_id):
    ...
    if not client:
        return jsonify({"error":"Cliente no encontrado"}), 400
    return jsonify("CLIENTE Eliminado CON ÉXITO"), 200
